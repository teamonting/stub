...
export default function withBrowsingContextNavigate<T extends object>({
  browsingContext
}: StubEnvironment): (value: T) => T & Pick<Stub, 'browsingContextNavigate'> {
  return value => ({
    ...value,
    async browsingContextNavigate(url: string, readinessState?: ReadinessState | undefined): Promise<NavigateResult> {
